...
past = 0
now = 0
bias = [101.92846918868916, 101.34821830287576, 100.05209034635068,
        100.33043707489742, 100.6644506859688, 102.09394537355578, 101.24078338508029]
a = 0.5
while count < 500000:
    count += 1
    # sleep(period)
    ...
    try:
        ...
        for i in range(N_sensors):
            if len(Ps[i]) > 0:
                tmp = (1-a)*Ps[i][-1] + a*(sensors[i].get("p")-bias[i])
            else:
                tmp = sensors[i].get("p")-bias[i]
            Ps[i].append(tmp)
