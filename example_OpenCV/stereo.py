...
if __name__ == "__main__":
    ...
    if cameras:
        frames = capture_camera_frames(interval=0.01, max_frames=100, camera_index=cameras[0])
        print("Captured frames:", len(frames))

        # Use the first frame to compute disparity
        if frames:
            left_image, right_image = split_frame(frames[0])
            disp = stereo_distance_measurement(left_image, right_image)
