...
class ina226:
    def __init__(self, ina226_addr=INA226_ADDRESS, i2c_bus_number=I2C_DEFAULT_BUS_NUMBER, i2c_clk_Khz=I2C_DEFAULT_CLK_KHZ, i2c_driver_type=I2C_DRIVER):
        ...
        if i2c_driver_type == 'AARDVARK':
            if PYTHON_AARDVARK_LIB_PRESENT is False:
                print(
                    'PYAARDVARK Driver is not installed, Please install and Try again.')
                sys.exit(0)
            ...
            if PYTHON_SMBUS_LIB_PRESENT is False:
                print(
                    'PYTHON SMBUS Driver is not installed, Please install and Try again.')
                sys.exit(0)
            ...
            if i2c_clk_Khz != I2C_DEFAULT_CLK_KHZ:
                print(
                    'Python SMBUS linux driver doesn\'t provide I2C CLK Freq Manipulation support yet,')
                print('So Ignoring i2c_clk_khz param and using default.')
        else:
            print(
                'Unknown I2C DRIVER Specified, available Options are : AARDVARK, SBC_LINUX_SMBUS')
    ...
    def readRegister16_SMBUS(self, register):
        ...
        data = self.i2c_bus.read_i2c_block_data(
            self.ina226_address, register, 2)
        higher_byte = data[0]
        lower_byte = data[1]
    ...
    def writeRegister16_SMBUS(self, register, dataWord):
        ...
        self.i2c_bus.write_i2c_block_data(
            self.ina226_address, register, [higher_byte, lower_byte])
    ...
    def readRegister16_AARDVARK(self, register):
        ...
        byte_char_data = self.i2c_bus.i2c_master_write_read(
            self.ina226_address, register_addr_str, 2)
...
def demo():
    try:
        ...
        while True:
            print("Current: "+str(round(iSensor.readShuntCurrent(), 3))+"A"+", Voltage: "+str(round(
                iSensor.readBusVoltage(), 3))+"V"+", Power:"+str(round(iSensor.readBusPower(), 3))+"W")
