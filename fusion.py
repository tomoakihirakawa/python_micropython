...
class fusion:
    ...
    def update2(self, Ain, Min, W, dt, num=0):
        ...
        alpha = 0.6
        self.A[1] = Normalize(
            Add(Times(self.A[0], alpha), Times(self.A[1], 1. - alpha)))
        self.M[1] = Normalize(
            Add(Times(self.M[0], alpha), Times(self.M[1], 1. - alpha)))
        A = self.A[1]
        M = self.M[1]
    ...
    def update00(self, A, M, W, dt, num=0):
        ...
        self.Q.set(
            (Add(self.Q, Times(Add(self.dqdt[0], self.dqdt[1]), dt/2.))))
    ...
    def update20(self, A, M, W, dt, num=0):
        self.dqdt = self.Q.d_dt(Times(W, dt))
        self.Q.set(Add(self.Q, self.dqdt))
    ...
    def dQ(self, A, M):
        return Quaternion(Minus(Dot(Inverse(self.dFdq(A, M)), self.F(A, M))))
    ...
    def dFdq(self, A, M):
        ...
        return [[2*(3*a2 + b2 + c2 + d2) + u2 - mc*u3 - ms*u5, 4*a*b + u1 - ms*u4, 4*a*c - u0 + ms*u3 - mc*u5, 4*a*d + mc*u4],
                [4*a*b + u1 - ms*u4, 2*(a2 + 3*b2 + c2 + d2) - u2 - mc *
                 u3 + ms*u5, 4*b*c - mc*u4, 4*b*d + u0 - ms*u3 - mc*u5],
                [4*a*c - u0 + ms*u3 - mc*u5, 4*b*c - mc*u4, 2 *
                    (a2 + b2 + 3*c2 + d2) - u2 + mc*u3 + ms*u5, 4*c*d + u1 - ms*u4],
                [4*a*d + mc*u4, 4*b*d + u0 - ms*u3 - mc*u5, 4*c*d + u1 -
                    ms*u4, 2*(a2 + b2 + c2 + 3*d2) + u2 + mc*u3 - ms*u5]]
